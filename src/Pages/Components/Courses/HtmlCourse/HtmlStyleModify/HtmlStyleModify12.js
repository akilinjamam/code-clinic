...
const HtmlStyleModify12 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData12 = {
            example1: example1,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/example1/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData12)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlStyleModify12;