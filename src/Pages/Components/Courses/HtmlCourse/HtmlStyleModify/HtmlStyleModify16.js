...
const HtmlStyleModify16 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData16 = {
            example5: example5,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/example5/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData16)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlStyleModify16;