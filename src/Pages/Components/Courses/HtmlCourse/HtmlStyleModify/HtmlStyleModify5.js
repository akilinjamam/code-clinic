...
const HtmlStyleModify5 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData5 = {
            title4: title4,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/title4/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData5)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlStyleModify5;