...
const HtmlStyleModify7 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData7 = {
            description1: description1,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/description1/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData7)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlStyleModify7;