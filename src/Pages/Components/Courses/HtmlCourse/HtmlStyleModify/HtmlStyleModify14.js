...
const HtmlStyleModify14 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData14 = {
            example3: example3,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/example3/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData14)
        })
            .then(res => {
                console.log(res)
                return res.json()
            ...
            <form onSubmit={handleModify} action="">
                <div className='p-5'>
                    ...
                    <div className='bg-gray-300 p-5 rounded'>
                        ...
                        <p> {dataaStyle.map(d => d.example3)} </p>
                        <br />
                        {dataaStyle && <div className=' flex'>
                            <textarea required type="text" name='example3' className="input input-bordered input-primary w-full " />
...
export default HtmlStyleModify14;