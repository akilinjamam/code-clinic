...
const HtmlStyleModify6 = () => {
    ...
    const handleModify = event => {
        ...
        const updateStyleData6 = {
            title5: title5,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlStyle/title5/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateStyleData6)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlStyleModify6;