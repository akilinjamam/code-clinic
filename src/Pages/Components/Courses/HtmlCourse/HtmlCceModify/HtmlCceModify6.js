...
const HtmlCceModify6 = () => {
    ...
    const handleModify = event => {
        ...
        const updateCceData6 = {
            title5: title5,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlCce/title5/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateCceData6)
        })
            .then(res => {
                console.log(res)
                return res.json()
