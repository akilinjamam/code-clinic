...
const HtmlImageModify8 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData8 = {
            description2: description2,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/description2/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData8)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify8;