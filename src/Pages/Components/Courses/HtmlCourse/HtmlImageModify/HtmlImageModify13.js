...
const HtmlImageModify13 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData13 = {
            example2: example2,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/example2/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData13)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify13;