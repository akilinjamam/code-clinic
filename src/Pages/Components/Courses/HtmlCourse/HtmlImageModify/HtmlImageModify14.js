...
const HtmlImageModify14 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData14 = {
            example3: example3,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/example3/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData14)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify14;