...
const HtmlImageModify10 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData10 = {
            description4: description4,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/description4/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData10)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify10;