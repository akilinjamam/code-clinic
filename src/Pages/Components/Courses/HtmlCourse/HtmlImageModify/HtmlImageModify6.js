...
const HtmlImageModify6 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData6 = {
            title5: title5,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/title5/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData6)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify6;